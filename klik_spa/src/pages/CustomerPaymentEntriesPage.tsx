...
interface InvoicePreviewProps {
  ...
  isB2C: boolean;
  currentDate: string;
  /** Per-item discount inputs keyed by item code/id, as entered in the cart. */
  itemDiscounts?: Record<string, { discountPercentage?: number; discountAmount?: number; customRate?: number }>;
  /** Final, tax-inclusive unit price per item code/id after per-item discounts are applied. */
  effectiveDisplayRateByItem?: Record<string, number>;
  /** Used here only to surface the whole-invoice "Bill Discount" as its own line. */
      ...
      <div className="space-y-2 mb-4">
        {cartItems.length > 0 ? (
          cartItems.map((item, index) => {
            const itemKey = item.item_code || item.id;
            const discountData = (itemKey && itemDiscounts[itemKey]) || itemDiscounts[item.id] || {};
            const hasCustomRate = discountData.customRate !== undefined && discountData.customRate !== null;
            const hasItemDiscount =
              Number(discountData.discountPercentage || 0) > 0 ||
              Number(discountData.discountAmount || 0) > 0 ||
              hasCustomRate;
            const effectiveRate = itemKey && effectiveDisplayRateByItem[itemKey] !== undefined
              ? effectiveDisplayRateByItem[itemKey]
              : item.price;
            const lineTotal = roundCurrency(effectiveRate * item.quantity);
            const lineDiscount = hasItemDiscount
              ? roundCurrency(Math.max(0, item.price - effectiveRate) * item.quantity)
              : 0;
            ...
            return (
              <div key={index} className="flex justify-between text-sm">
                <div className="flex-1">
                    ...
                    {hasItemDiscount ? (
                      <>
                        <span className="line-through text-gray-400 dark:text-gray-500 mr-1">
                          {formatCurrencyWithSymbol(item.price, displayCurrencySymbol)}
                        </span>
                        <span className="text-green-600 dark:text-green-400 font-medium">
                          {formatCurrencyWithSymbol(effectiveRate, displayCurrencySymbol)}
                        </span>
                      </>
                    ) : (
                      formatCurrencyWithSymbol(item.price, displayCurrencySymbol)
                  ...
                  {hasItemDiscount && lineDiscount > 0 && (
                    <p className="text-xs text-green-600 dark:text-green-400">
                      Item discount: -{formatCurrencyWithSymbol(lineDiscount, displayCurrencySymbol)}
                    </p>
                  )}
                </div>
                <p className="font-medium text-gray-900 dark:text-white">
                  {formatCurrencyWithSymbol(lineTotal, displayCurrencySymbol)}
                </p>
              </div>
            );
          })
